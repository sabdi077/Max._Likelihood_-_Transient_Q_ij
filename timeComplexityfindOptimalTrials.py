...
def find_min_trials(real_alphas, real_betas, num_mice, num_est, Q_i, threshold, success_rate, max_trials):
    ...
    for t in range(1, max_trials + 1):
        alpha_condition = np.mean(MSE_ALPHA_matrix[:, t-1] <= threshold) >= success_rate
        beta_condition = np.mean(MSE_BETA_matrix[:, t-1] <= threshold) >= success_rate
...
def representative_segment_old(num_iterations):
    start_time = time.time()
    
    # Old code representative segment
    num_mice = 10
    ...
    threshold = 0.04
    success_rate = 1
    max_trials = num_iterations  # Use a smaller number for the test
    num_est = 10
    ...
    print(f"Old Segment Computation Time: {segment_time} seconds")
    return segment_time

# Measure the time for a representative segment
old_segment_time = representative_segment_old(100)  # Use a smaller number for the test



def representative_segment_new(num_iterations):
    start_time = time.time()
    
    # Optimized code representative segment
    num_mice = 10
    real_alphas = np.linspace(0, 1, num_mice)
    real_betas = np.linspace(0, 10, num_mice)
    ...
    end_time = time.time()
    segment_time = end_time - start_time
    print(f"New Segment Computation Time: {segment_time} seconds")
    return segment_time

# Measure the time for a representative segment
new_segment_time = representative_segment_new(100)  # Use a smaller number for the test


# Total number of iterations in the full code
total_iterations = 5000

# Estimate total computation times based on the segment times
estimated_T1 = (total_iterations / 100) * old_segment_time
estimated_T2 = (total_iterations / 100) * new_segment_time

# Calculate the speedup factor
speedup_factor = estimated_T1 / estimated_T2
print(f"Estimated Speedup Factor: {speedup_factor}")
